...
export default class ImageLoader {
    ...
    loadImagesFromLocalAPI(id: number, similarityCriteria: number[], setter: (images: any) => void) {
        axios.get(`http://localhost:8000/similars/?baseId=${id}&imageCount=7&${similarityCriteria.map((n) => `similarityCriteria=${n}`).join("&")}`, {
